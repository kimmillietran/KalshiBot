...
import { MomentumGovernedDiscoveryError } from "./momentumDiscoveryTypes";
 ...
 */
export function createTrainOnlyMomentumDiscoveryIo(input: {
    ...
    for (const blocked of quarantined) {
      if (normalized === blocked || normalized.startsWith(`${blocked}/`)) {
        throw new MomentumGovernedDiscoveryError(
          `Train-only momentum discovery IO refused to read quarantined capture path: ${path}`,
        );
      }
    }
  }
