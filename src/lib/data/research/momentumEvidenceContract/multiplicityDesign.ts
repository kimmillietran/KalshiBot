...
export function buildMomentumMultiplicityDesign(): {
  ...
  return {
    discoveryHypothesisCount: EXPECTED_MOMENTUM_DISCOVERY_HYPOTHESIS_COUNT,
    maxValidationShortlistK: MOMENTUM_MAX_SHORTLIST_K,
    lockedHoldoutCandidates: 1,
    untouchedHoldoutEvaluations: 1,
    lineage:
    ...
    whereCorrectionApplies: [
      {
        ...
        policy:
          "Retain full 12-hypothesis multiplicity lineage forever. Shortlist selection is "
          + "multiplicity-aware governance (K≤3), not pretending a single hypothesis was ever "
          + "considered. Reuse repository OOS/FDR-style discipline for exploratory TRAIN claims.",
      },
      {
        stage: "Validation",
        policy:
          "Family size = |shortlist| ≤ 3. Validation is replication/narrowing on a frozen "
          + "shortlist; do not re-open the 12-cell grid or mutate W/X/H/direction.",
      },
      {
        stage: "Holdout",
        policy:
