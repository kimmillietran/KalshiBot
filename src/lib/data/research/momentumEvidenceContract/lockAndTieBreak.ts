import type { MomentumCandidateDefinition } from "./momentumEvidenceContractTypes";
import type { MomentumValidationCandidateResult } from "./validationSemantics";

export type MomentumLockedHoldoutCandidate = {
  candidate: MomentumCandidateDefinition;
  lockRationale: string[];
  survivorCount: number;
};

export type MomentumLockInputSurvivor = MomentumValidationCandidateResult & {
...
export function buildMomentumLockPolicy(): {
  ...
  return {
    ...
    tieBreakOrder: [
      ...
      "effect clears material floor (abs magnitude only soft margin)",
      "structural simplicity (smaller W → smaller X → smaller H)",
      "candidate ID",
    ],
    notLargestEffectOnly: true,
    holdoutEvaluatesExactlyOne: true,
...
export function lockHoldoutCandidateFromValidationSurvivors(input: {
  survivors: readonly MomentumLockInputSurvivor[];
}): MomentumLockedHoldoutCandidate | null {
  ...
  return {
    ...
    lockRationale: [
      ...
      ...buildMomentumLockPolicy().tieBreakOrder.map((step, i) => `${i + 1}. ${step}`),
    ],
  };
}
