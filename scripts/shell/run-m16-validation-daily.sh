#!/usr/bin/env bash
# M16.2a governed daily validation runner wrapper (macOS launchd).
...
if [[ -z "${ENV_LOADER}" ]]; then
  echo "M16.2a: missing load-kalshi-env.sh (expected KalshiBot primary or repo root)" >&2
  exit 1
fi
...
if [[ -z "${KALSHI_API_KEY_ID:-}" || -z "${KALSHI_API_PRIVATE_KEY_PATH:-}" ]]; then
  echo "M16.2a: Kalshi credentials not loaded" >&2
  exit 1
fi
...
echo "M16.2a launch ${STAMP} repo=${REPO_ROOT} caffeinate=1 live=1" >> "${LOG_DIR}/wrapper.log"
