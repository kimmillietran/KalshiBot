...
function createCompleteImportFixture() {
  ...
  return {
    ...
    directories: new Set([root, series, marketDir]),
  };
}
...
describe("runBuildDatasetManifestCommand", () => {
  it("writes dataset-manifest.json for organized imports", () => {
    const { root, files, directories } = createCompleteImportFixture();
    const { io, writes, getStdout } = createIo(files, directories);
  ...
  it("accepts npm-stripped positional input-dir and output", () => {
    const { root, files, directories } = createCompleteImportFixture();
    const { io, writes } = createIo(files, directories);
