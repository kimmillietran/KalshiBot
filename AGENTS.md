...
- The dev server is the only service needed. The app's `src/app/api/*` BFF routes proxy to **public** external APIs (Coinbase for the live BTC price/candles, Kalshi for live BTC markets/pricing). No API keys/secrets are required for the core dashboard, but live UI use needs outbound internet. If a restricted-egress environment is ever configured, allowlist `api.exchange.coinbase.com`, `external-api.kalshi.com`, and `api.kraken.com` (the Kraken failover BTC provider) so live UI testing keeps working. Lint/test/build need no allowlist.
