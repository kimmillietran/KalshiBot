...
| `btc.price` | Latest `btcBars[].closeUsd` |
| `btc.candles` | All `btcBars` mapped to OHLC + `closeTime` timestamp |
| `btc.change24hPercent` | `null` (not present in historical bars) |
| `pricing` | Latest `kalshiCandles[]` bid/ask; mids derived from quotes |
| `pricing.liquidityQuality` | Derived from merged silver `qualityFlags` |
