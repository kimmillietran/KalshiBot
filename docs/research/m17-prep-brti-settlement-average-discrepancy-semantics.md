...
| Is `avg_60s_data` official expiration? | **Not stated** by vendor. Empirically, diagnostic half-even 2dp matched official on **three** observed closes (04:15Z committed; 23:15Z v2; 00:00Z v3 with 60 verified 1Hz samples and exact-decimal mean agreement). `last_60s_windowed_average_15min` did not match — a **strong empirical candidate only**, not a confirmed binding or rounding rule | S1; PR116 v3; one-close v2/v3 | Exact 1Hz selection + official rounding open; Kalshi support non-blocking | Do not select a field for gates; keep reporting both; use research `SettlementEstimate` statuses |
